...
### Phase 0.5：デスクトップリサーチ + 競合候補洗い出し

```
input:  Phase 0から推定した業種・地域・キーワード
tool:   web_search（Brave Search MCP or Claude Code標準）
        Ahrefs MCP（利用可能な場合：site-explorer-metrics, organic-competitors等）
output: 市場仮説・競合候補・規制有無・トレンド（叩き台として提示）
        競合4層マッピング（直接/間接/代替行動/ベンチマーク）
注意:   結果はすべて「仮説」として扱う。Phase 1で覆ることが前提
参照:   references/competitor-analysis-guide.md
```
...
### Phase 1.5：専門家選定 + 競合CEO選定

```
input:  Phase 1で収集した情報 + Phase 0.5の競合候補
process: expert-library.md の選定マトリクスを参照
         competitor-analysis-guide.md で競合3-5社を確定
         各社の深堀り分析（7カテゴリ）を実施
         competitor-ceo-template.md で仮想CEOペルソナを生成
output: 推奨専門家リスト → 経営者が承認 → 参加メンバー確定
        競合CEO 3-5名のペルソナ → 経営者が承認 → 会議参加メンバー確定
```

### Phase 2：専門家会議 + 競合CEOラウンド

```
input:  Phase 1の情報 + 確定した専門家リスト + 競合CEOペルソナ
process: expert-personas.md + expert-library.md + facilitator-roles.md を参照
         competitor-ceo-template.md + agent-prompts/competitor-ceo-round.md を参照

ラウンド構成：
  R1: 自社チーム独立分析（Agent並列起動）→ オーナー反応
  R1.5: 競合CEOラウンド（Agent並列起動）→ オーナー反応
  R2: 自社チーム クロスグリリング（R1 + 競合CEOの指摘を踏まえて）
  R3: 統合

競合CEOのグリリングモード（経営者が選択可能）：
  - 率直モード（デフォルト）: 脅威は認める、余裕も認める
  - 攻撃モード: 本気で市場を奪いにくる前提
  - 先輩モード: ベンチマーク企業のCEO用、建設的アドバイス中心

output: 各専門家のフィードバック（🔴/🟡/🟢形式）
        競合CEOの率直な評価（脅威度/されると困ること/されても困らないこと/不毛な戦い）
        補佐CEO介入（コンフリクト時）
        通訳（用語平易化）
...
| `agent-prompts/competitor-ceo-round.md` | 競合CEO仮想グリリング（Phase 2 / 4.5 / 6） |
...
競合・ベンチマーク企業の仮想CEOを経営会議に参加させ、自社の計画を「敵の目線」で検証する仕組み。

### フロー

1. **Phase 0.5**: 競合候補の4層マッピング（直接/間接/代替行動/ベンチマーク）→ `references/competitor-analysis-guide.md`
2. **Phase 1**: オーナーと競合認識をすり合わせ（「一番意識している競合は？」「顧客が代わりに使うものは？」）
3. **Phase 1.5**: 競合3-5社を確定。各社の深堀り分析（7カテゴリ）→ 仮想CEOペルソナ生成 → `personas/competitor-ceo-template.md`
4. **Phase 2**: 自社チームR1完了後に「競合CEOラウンド」としてAgent並列起動 → `agent-prompts/competitor-ceo-round.md`
5. **Phase 4.5**: ストレステストで競合CEOも並列起動（「この計画を実行されたら、うちはどう対抗するか」）
6. **Phase 6/6E**: 計画更新時・競合の動きがあった時に仮想CEOのペルソナを更新して再召集

### 競合CEOの発言で得られるもの

| 発言内容 | 経営者にとっての意味 |
|---------|-------------------|
| 「されると困る」 | → 本当の競争優位。ここにリソースを集中すべき |
| 「されても困らない」 | → 差別化が弱い。ここで戦うのは非効率 |
| 「ここで戦うのは不毛」 | → 消耗戦を避けるべき領域 |
| 「うちの競争資源はここ」 | → 相手の強みの本質。正面衝突を避ける |
| 「棲み分けできる」 | → エコシステム共存の可能性 |
| 「うちならこう対抗する」 | → 事前に備えるべき競合アクション |

### グリリングモード

| モード | Layer | トーン |
|--------|-------|--------|
| 率直モード（デフォルト） | L1/L2 | 敵だけど正直。脅威は認める、余裕も認める |
| 攻撃モード | L1 | 本気で市場を奪いにくる前提。弱点を容赦なく突く |
| 先輩モード | L4 | ベンチマーク企業向け。建設的アドバイス中心 |

経営者がモードを選択可能。L4ベンチマークは自動的に先輩モード。

---

## 15. Dual-Phase Prompt Architecture（PRISM知見の適用）

### 背景

PRISM研究（Persona Routing via Intent-based Self-Modeling）により、
LLMのペルソナプロンプトには「タスクの二面性」があることが判明：

- **事前学習依存タスク**（事実検索・計算・論理推論）→ ペルソナが精度を**低下**させる
- **アライメント依存タスク**（トーン・形式・安全性・視点）→ ペルソナが精度を**向上**させる

本システムの専門家プロンプトはこの両方のタスクを含むため、
PRISMのGated Routing概念をプロンプト設計レベルで実装する。

### 二層分離の原則

```
Phase A（客観分析）— ペルソナなし
  目的：事実検索・数値計算・ベンチマーク比較・リスク定量化
  ルール：キャラクターを演じない。「思います」「感じます」禁止。
         検証可能な事実と計算のみ。
  効果：事前学習の知識検索能力を最大限に活用

Phase B（専門家解釈）— ペルソナ全開
  目的：Phase Aの分析結果を専門家視点で解釈・通訳・質問生成
  ルール：ペルソナ定義に従い、トーン・形式・対立軸を表現
         Phase Aの数字を必ず引用しながら議論
  効果：アライメント能力（トーン・形式・視点制御）を最大限に活用
```
...
| プロンプト | 適用 | 理由 |
|---------|------|------|
| `round1-expert.md` | **二層分離** | 事実検索 + 解釈の混在が最も多い |
| `stress-test-expert.md` | **二層分離** | 数値検証の正確性が核心 |
| `financial-review-expert.md` | **二層分離** | 計算精度が最重要 |
| `competitor-ceo-round.md` | **二層分離** | 競合の実データ引用が核心 |
| `round2-cross-grilling.md` | ペルソナ全開 | 対立・議論・トーンが核心（アライメント依存） |
| `round3-integration.md` | ペルソナ全開 | 統合・形式・構造化が核心（アライメント依存） |
| `kpi-check-in.md` | ペルソナ全開 | 短い形式出力が核心（アライメント依存） |

### 参考文献

`appendix/PRISM_Gated_Persona_Routing.pdf` — NotebookLMによるPRISM研究の解説資料
